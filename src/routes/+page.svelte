	...
	{#if mode !== 'list'}
		...
		<div
			class={isFormExpanded
				? 'flex-1 min-h-0 overflow-auto px-6 py-4'
				: 'fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none'}
		>
			<div
				class={isFormExpanded
					? 'mx-auto max-w-[1600px] rounded-lg border border-warm-200 bg-white p-6 shadow-sm'
					: 'pointer-events-auto relative max-h-[90vh] w-full max-w-[72rem] overflow-auto rounded-xl bg-white p-6 shadow-2xl'}
				onclick={(e) => {
					if (!isFormExpanded) e.stopPropagation();
				}}
				role={isFormExpanded ? 'region' : 'dialog'}
				aria-modal={isFormExpanded ? undefined : 'true'}
				aria-label={isFormExpanded ? 'Incident editor' : undefined}
				aria-labelledby={isFormExpanded ? undefined : 'incident-form-title'}
				tabindex={isFormExpanded ? undefined : -1}
			>
				<div class="mb-4 flex items-center {isFormExpanded ? 'justify-start' : 'justify-end'}">
					{#if isFormExpanded}
						<button
							...
							class="rounded-lg border border-warm-200 bg-white px-4 py-2 text-sm text-warm-700 hover:bg-warm-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500"
						>
							Back to list
						</button>
					{:else}
						<button
							...
							class="rounded-lg border border-warm-200 bg-white px-3 py-1.5 text-sm text-warm-700 hover:bg-warm-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500"
						>
							Expand
						</button>
					{/if}
